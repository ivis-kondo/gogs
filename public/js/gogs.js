...
function initRepository() {
  ...
  // Custom GIN code: #repo-clone-gin
  ...
  // Custom GIN code: #download-repo-button
...
function setCodeMirror($editArea) {
  ...
  // GIN specific code: codeMirrorEditor.on("keyup"...)
...
$(document).ready(function() {
  ...
  if ($dropzone.length > 0) {
    ...
    $dropzone.dropzone({
      ...
      init: function() {
        ...
        // GIN specific code: this.on("sending", ...)
        // Used for directory tree structure upload; tree would be flattened otherwise
        this.on("sending", function (file, xhr, formData) {
          formData.append("full_path", file.fullPath);
        });
        // GIN specific code: this.on("addedfile", ...)
        // Used for file display in directory trees
  ...
  initAdmin();
  initCodeView();
  initCookieWarn(); // GIN specific code
  initSuggest(); // GIN specific code
  ...
  if ($("#repo-clone-url").length > 0) {
    switch (localStorage.getItem("repo-clone-protocol")) {
      case "ssh":
        ...
        break;
      default:
        $('#repo-clone-gin').click(); // GIN specific code; shows "gin get ..." as default repo clone command.
        break;
    }
  }
